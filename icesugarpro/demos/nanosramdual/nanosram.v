module top(
    ...
    localparam
        ...
        RUNLEN     = 32;   			// how many bytes to transfer (must divide the size of the PSRAM)
    ...
    // assign CS pins
    // during init sram_cs goes to both
    // after init the first 8MB is IC #1 and second 8MB is IC #2
    always @(*) begin
		if (~rst_n | ~sram_is_init) begin
			// during init we send the EQIO to both chips
			cs_pin  = sram_cs;
			cs2_pin = sram_cs;
		end else begin
			// 8MB chips...
