...
// PSRAM (nanosram + 8KB nanocache mapped at 0x08000000, we offset 64KB into PSRAM so we're not overwriting the demo itself)
...
void main(void)
{
    ...
    *VGA_CTRL = CTRL_MODE_GFX | (active_page ? CTRL_PAGE_1 : 0);
    ...
    // Off-screen canvas in PSRAM (uses 64KB starting at address 0x08010000)
    ...
    while (1) {
        ...
        // -------------------------------------------------------------
        // STEP 1: Render scene into PSRAM (0x08010000)
        // -------------------------------------------------------------
