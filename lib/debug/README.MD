...
reading and writing BITS-bit payloads (BITS is a parameter to the modules which defaults to 128) with arbitrary addressing.  
The theory of operation is you'd map useful things to a wire that you send as "debug_outgoing_data" and then you can react
to things on "debug_incoming_data" when the toggle "debug_incoming_tgl" changes value from one clock to the next.
...
## Identities
Each node also gets a BITS-bit "identity" that is meant to be used by the host debugger to tell what the node is.  The identity net
...
whatever.  In reality it's just BITS bits and it's up to the design to specify how to parse that.

## Operation
The debugger uses a store-and-forward architecture where each node first receives and entire (BITS+16)-bit packet (15-bit address, read/write command, BITS-bit
...
## Using
Each packet is BITS+16 bits long made up by a direction bit [0], address [15:1], and payload [BITS+15:16].  A direction bit of 0 means read, a direction
of 1 means write.  The address 7FFF is a broadcast meaning this supports upto 32,766 nodes.

When a read matches the address of a node, the value of "debug_outgoing_data" is placed in the payload field and forwarded on.  The signal
debug_outgoing_tgl is inverted to tell the module that the debug node read the value.  Once the toggle is changed the module itself can change the
debug_outgoing_data since it's latched internally in the debug node to send out to the next in the chain.  The payload field is used by the node to indicate 
commands in the first byte of the payload [23:16].  A value of 0 means "read identity" anything else just copies the outgoing data.  
...
## Fetch Identity
The identity field is read by sending a read request with the command byte [23:16] set to zero.  The BITS-bit payload that is fetched is design
...
## Data Throughput
At a minimum a packet takes (BITS+16) * 4 cycles to transmit (e.g. 576 with 128-bit payloads).  So if you have N nodes with 128-bit payloads it will take 
576 * N cycles to transit the entire chain.  As a result this is really not meant to be a high speed bus.  It's meant to be small and lightweight and easy to 
embed deep inside designs.
...
node** in the chain, and attach debug_tx_* to the **output of the LAST node in the chain**.  The bridge reads (BITS+16)/8 bytes at a time from the UART RX
pin, sends it over the simple serial bus, reads the (BITS+16)/8 byte output and sends it to the UART TX pin.  
