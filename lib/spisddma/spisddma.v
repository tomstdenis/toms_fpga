...
module spisddma #(
    ...
    wire [63:0] spi_cmd_block;                    // Combined bus aggregating the structural layout above
    ...
    always @(posedge clk) begin
        if (!rst_n) begin
            ...
            case(state)
                ...
                STATE_INIT_CMD55:
                    begin
						setup_spi_cmd(8'd55, 32'h0, 8'h00, STATE_INIT_CMD55_R1);
                    end
                ...
                STATE_INIT_CMD55_R1:
                    begin
                        if (temp_wire_bits != 8'h01) begin
                            ...
                            setup_spi_cmd(8'd41, 32'h40000000, 8'h00, STATE_INIT_ACMD41_R1);
                        end
                    end
                ...
                STATE_INIT_CMD58:
                    begin
						...
						setup_spi_cmd(8'd58, 32'h0, 8'h00, STATE_INIT_CMD58_R1);
                    end
                ...
                STATE_SEND_CMD:
                    begin
                        temp_wire_bits <= spi_cmd_block[56 - (state_step * 8) +: 8];
                        mosi_pin       <= spi_cmd_block[63 - (state_step * 8)];
                        bit_cnt        <= bit_cnt_orig;
                        sck_timer      <= sck_timer_orig;
                        state          <= STATE_SHIFT_DATA;
                        tag            <= (state_step == 7) ? STATE_READ_R1 : STATE_SEND_CMD;
                ...
                STATE_WRITE_CRC:
                    begin
						if (state_step == 2) begin
							...
							mosi_pin       <= cmd_crc16[15 - (state_step * 8)];
                            tag            <= state;
							state          <= STATE_SHIFT_DATA;
						end
