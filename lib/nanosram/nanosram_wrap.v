...
module nanosram #(
    ...
    parameter SET_WRAP_MODE=1,				// set wrapped mode 8'hC0
                    ...
                    if (PSRAM == 0) begin
						...
						if (SET_WRAP_MODE == 0) begin
							...
							case (init_en[0])
								...
								1'b0: // WRAP (1 byte command in QPI mode, 8'hC0;
									begin
										init_sr        <= {32'h0};
										temp_wire_bits <= 4'h0;			// 2nd nibble of C0
										sio_dout       <= 4'hC;			// 1st nibble of C0
										init_cnt       <= 0;			// only 1 byte		
									end
							endcase
						end
                    end
...
`endif
                        ...
                        if (~temp_cnt) begin
                            ...
                            if (init_cnt != 0) begin
								...
								if (SET_WRAP_MODE == 1) begin
									init_en        <= {init_en[0], 1'b0};
								end else begin
									init_en[1]     <= 1'b0;
								end
            ...
            FSM_DELAY:
                begin
                    ...
                    if (delay_timer == 0) begin
                        ...
                        fsm_state   <= {1'b0, init_en[1]};            // jump to EQIO if init_en, or IDLE if not
                    end
                end
        endcase
        if (!rst_n) begin
			...
			temp_cnt      <= 1;
			if (SET_WRAP_MODE == 1) begin
				init_en   <= 2'b11;			// send EQIO and WRAP
			end else begin
				init_en[1] <= 1'b1;			// only send EQIO
			end
`ifdef MODEL_SIM
			sim_active    <= 1'b0;
`endif			
