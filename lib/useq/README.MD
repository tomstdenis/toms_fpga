...
1024 byte ROM space.  The core features two instruction sets (EXEC1 and EXEC2) that are designed
...
correctly while running EXEC2 code normally.  Once in the ISR the core disables intterupts until a RTI
...
On my Tang Nano 20K (GW2AR-18) I get the following sizes/times (keep in mind these should be treated as estimates and depend on route/place settings...)

	- All enabled: 85~94MHz, 1110 CLS, 1605 LUT, 129 ALU, 128 ROM16, 259 Registers
...
Fun ideas:
	...
	- along the lines of banking, we would make JMPA/CALLA and JMP imm/CALL imm use a different convention where the 8-bit target is actually
	2-bits bank + 6-bits offset * 4, so you could jump/call any 4-byte aligned target in upto 4 banks.  In this configuration you'd have 1Kbyte of
	code space.  We'd have to make sure mem_addr/PC/LR/ILR are 10 bits but that's a trivial change.  Probably for sanity just make them 10-bits always and then
	make the interpretation of the opcodes a parameter.
...
The ROM16s (configured as 8x1024) is loaded at synthesis time and once it comes out of reset
...
80: JMP imm			PC = imm << 2
81: SJMP imm		PC = PC + { imm[7], imm[7], imm }; // short unaligned jumps
82: JZ imm			if !A then PC = imm<<2
83: SJZ imm			if !A then PC = PC + { imm[7], imm[7], imm };  // short unaligned jumps
84: JNZ imm			if A then PC = imm, else PC += 2
85  SJNZ imm		if A then PC = PC + { imm[7], imm[7], imm }; // short unaligned jumps
86: CALL imm 		LR = PC + 2, PC = ROM[PC+1] << 2
87: LDI imm			A = ROM[PC+1], PC = PC + 2
88: ADDI imm		A = A + ROM[PC+1], PC = PC + 2
89: SUBI imm		A = A - ROM[PC+1], PC = PC + 2
8A: EORI imm		A = A ^ ROM[PC+1], PC = PC + 2
8B: ANDI imm		A = A & ROM[PC+1], PC = PC + 2
8C: ORI imm			A = A | ROM[PC+1], PC = PC + 2
8D: LDR0 imm		R[0] = imm, PC += 2
8E: LDR1 imm		R[1] = imm, PC += 2;
8F: XXX 
90: INC, 			A = A + 1, PC += 1
91: DEC, 			A = A - 1, PC += 1
92: ASL, 			A = A << 1, PC += 1
...
9A: NOT, 			A = ~A, PC += 1
9B: CLR, 			A = 0, PC += 1
9C: LDA, 			R14 = A + 1, A = ROM[A], PC += 1 (R14 stores a pointer to the next byte you can fetch with "LD E")
9D: SIGT, 			PC = PC + 1 + (A > R[0]) ? 1 : 0   // skip if greater
9E: SIEQ, 			PC = PC + 1 + (A == R[0]) ? 1 : 0  // skip if equal
9F: SILT, 			PC = PC + 1 + (A < R[0]) ? 1 : 0   // skip if less than
Ax: XXX FREE
Bx: XXX FREE
Cx: XXX FREE
Dx: XXX FREE 
...
E5: NEG				A = -A, PC += 1
E6: SEI, 			PC += 1, int_mask = A, if i_port & int_mask is a posedge (prev < cur) (from registered copy) then IRQ
E7: NOP
E8: HLT				PC = PC
E9: RET, 			PC = LR
EA: RTI, 			PC = ILR, re-enable interrupts and return
EB: WAIT0,			PC = PC + (i_port[R1[2:0]]) ? 0 : 1							<---- since WAIT's are likely clock related we use R1 to address the pin here
EC: WAIT1,			PC = PC + (i_port[R1[2:0]]) ? 1 : 0
ED: EXEC2 			Byte that follows PC is from the EXEC2 table
...
D3r: LDA r				R[r] = A, PC += 1  // technically redundant as you can just XCH or do CLR r, ADDA r
