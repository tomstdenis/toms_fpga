...
86: LDIR0 imm		R[0] = imm, PC += 2
87: LDIR1 imm		R[1] = imm, PC += 2;
88: LDIR11 imm		R[11] = imm, PC += 2;
89: LDIR12 imm		R[12] = imm, PC += 2;
8A: LDIR13 imm		R[13] = imm, PC += 2;
8B: LDIR14 imm		R[14] = imm, PC += 2;
8C: MUL				{R[0], A} = A * R[0], PC += 1
8D: LDM 			A <= ROM[{R[14], R[13]}, {R[14], R[13]} <= {R[14], R[13]} + 1'b1;   (ram and rom are the same address space)
8E: STM				RAM{R[12], R[11]} <= A, {R[12], R[11]} <= {R[12], R[11]} + 1'b1;    
8F: LDMIND			R[14]:R[13] <= MEM[R[14]:R[13] + A * 2], PC += 1					(load pointer indirect indexed off A)
...
E2: TGLBIT, 		o_port[R0[5:3]] ^= 1, PC += 1    // fast toggle-1			<--- things clock pin related use R1 to address the pin
E3: IN, 			A = i_port, PC += 1
E4: INBIT, 		 	A = i_port[R0[2:0]] ? 1 : 0 , PC += 1
E5: NEG				A = -A, PC += 1
E6: PUSHA			STACK[SP++] = A, PC += 1
E7: SEI imm			int_mask = imm, PC += 2
E8: SAI imm			int_vect = imm << 4; PC += 2;
E9: POPA			A = STACK[--SP], PC += 1
EA: RET, 			PC = STACK[SP], SP += 1;
EB: RTI, 			PC = ILR, re-enable interrupts and return
EC: WAIT0,			PC = PC + (i_port[R0[5:3]]) ? 0 : 1							<---- since WAIT's are likely clock related we use R1 to address the pin here
ED: WAIT1,			PC = PC + (i_port[R0[5:3]]) ? 1 : 0
